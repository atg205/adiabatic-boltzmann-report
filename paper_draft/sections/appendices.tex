...
\hline
T1. Conditional-MLE thermometry (Sec.~\ref{sec:cem-mle}) & \textbf{Partially closed} & Implemented here and numerically compared against \texttt{estimate\_beta\_eff\_cem} (\texttt{src/encoder.py:206--219}) on fresh Gibbs samples from the surviving \texttt{checkpoints/cem\_validation/} models: both estimators are unbiased, MLE has $\sim\!15$--$20\%$ lower variance once the network is partially trained. Untested on any real annealer sample, since no run's raw joint $(v,h)$ was ever logged. \\
