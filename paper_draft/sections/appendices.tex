...
\hline
T1. Conditional-MLE thermometry (Sec.~\ref{sec:cem-mle}) & \textbf{Preliminary} & Implemented and validity-checked in App.~\ref{app:mle-check} against \texttt{estimate\_beta\_eff\_cem} (\texttt{src/encoder.py:206--219}) on fresh Gibbs samples from the surviving \texttt{checkpoints/cem\_validation/} models: both estimators are unbiased, MLE has $\sim\!15$--$20\%$ lower variance once the network is partially trained. This is a synthetic sanity check only -- $\widehat\beta_{\mathrm{MLE}}$ has not replaced $\widehat\beta_{\mathrm{LS}}$ in any training loop, and untested on any real annealer sample, since no run's raw joint $(v,h)$ was ever logged. \\
...
\section{Conditional-MLE estimator: a validity check}
\label{app:mle-check}

This appendix reports a minimal check that the conditional-MLE thermometry estimator of Sec.~\ref{sec:cem-mle} (Eq.~\eqref{eq:score}) is not broken, before anyone builds on it further. It is not evidence that the estimator helps on real hardware: $\widehat\beta_{\mathrm{MLE}}$ has never replaced $\widehat\beta_{\mathrm{LS}}$ in the actual training loop, and every QPU-trained result reported elsewhere in this paper used the least-squares estimator exclusively.

No archived annealer run retains its raw joint $(v,h)$ samples, so a direct comparison on recorded QPU data is not possible. A separate, pre-existing validation study (\texttt{scripts/exper/cem\_validation\_sweep.py}) left three trained-RBM checkpoints on disk ($N=8$, $h=1.0$, early/mid/late training stages) together with the classical Gibbs sampler used there to test $\widehat\beta_{\mathrm{LS}}$ against an exact visible-marginal reference $\widehat\beta_{\mathrm{KL}}$. That Gibbs sampler targets the RBM's own conditionals with no rescaling (confirmed by reading \texttt{src/sampler.py}'s \texttt{\_gibbs\_sample}: $p(h\mid v)=\sigma(2(b+Wv))$, no $\beta_x$ anywhere), so its true generating $\beta$ is exactly $1$, not merely estimated -- letting all three estimators be scored against a known answer instead of against each other.

We implemented Eq.~\eqref{eq:score} and drew $150$ independent fresh samples per checkpoint from the surviving models (CPU only; $N=8$ makes this sub-second work). Figure~\ref{fig:mle-vs-ls} shows the result: both estimators are essentially unbiased at all three training stages (bias $\le0.01$ in $\beta$), and $\widehat\beta_{\mathrm{MLE}}$ has a modest but consistent variance reduction over $\widehat\beta_{\mathrm{LS}}$ once the network is at least partially trained -- standard deviation $0.042$ vs.\ $0.053$ at ``mid'' and $0.046$ vs.\ $0.056$ at ``late'' -- with no detectable difference at the untrained ``early'' checkpoint. This is a modest effect, not a large gain, and it tests only the well-specified case (samples genuinely drawn from the model's own Gibbs law at $\beta=1$); it says nothing about the misspecified regime a real annealer's non-Gibbs output represents, which is what Sec.~\ref{sec:cem-theory} still leaves open.
...
\paragraph{Status.} \textbf{T1: preliminary.} The two estimators are now numerically compared, with a real (if modest) efficiency result, on a controlled synthetic reference -- but this is a first look, not a settled conclusion: it is one $(N,h)$ cell, one sampler, and does not yet warrant retraining any real QPU/classical run with $\widehat\beta_{\mathrm{MLE}}$ in place of $\widehat\beta_{\mathrm{LS}}$. Whether that larger step (re-running the paper's actual benchmark cells with the new estimator) is worth the cost is an open decision, not resolved by this appendix. What is unchanged from before: no real annealer run has ever had its raw joint samples logged, so whether $\widehat\beta_{\mathrm{MLE}}$'s advantage over $\widehat\beta_{\mathrm{LS}}$ persists -- or reverses -- under actual QPU non-Gibbs distortion is still untested.
...
\begin{figure}[h]
\centering
\panel{.48}{\textbf{(a)}\\\includegraphics[width=\linewidth]{figures/sparsity_exact_budget.pdf}}\hfill
\panel{.48}{\textbf{(b)}\\\includegraphics[width=\linewidth]{figures/sparsity_matched_iterations.pdf}}
...
\begin{figure}[h]
\centering
\panel{.47}{\textbf{(a)}\\\includegraphics[width=\linewidth]{figures/parallel_embedding/parallel_embedding_np4_pegasus.pdf}}\hfill
\panel{.50}{\textbf{(b)}\\\includegraphics[width=\linewidth]{figures/parallel_embedding/parallel_embedding_np_seeds_qpu_time.pdf}}
