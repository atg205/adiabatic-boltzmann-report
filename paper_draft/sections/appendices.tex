...
T5. QPU hardware-resource telemetry (Sec.~\ref{sec:hw-resources}) & \textbf{Partially closed} & Qubit count and chain length (Table~\ref{tab:resources}) recovered from archived \texttt{embedding\_info} ($N=8,32,64$) plus an offline recomputation for $N=16$ from the cached live hardware-graph snapshot -- no QPU access. Chain-break fraction, embedding success rate, and programming/anneal/readout timing breakdown remain absent: the D-Wave sampling paths in \texttt{src/sampler.py} (e.g.\ lines $\approx$1478, $\approx$1588) read only \texttt{info["timing"]["qpu\_access\_time"]}, and those fields are only obtainable from a live QPU call. \\
