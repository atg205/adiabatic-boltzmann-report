...
len(beta_x_values) * 2 * n_repeats calls, i.e. a few seconds of QPU time
total for the defaults below. A budget guard (same pattern as
scripts/exper/dwave_matched_sweep.py) still checks src/../time.json before
every call and aborts rather than silently overspending.
...
def _make_plots(beta_x_values, dtv_results, beta_results, floor, N, h, n_reads, out_dir):
    setup_style(fontsize=14, scale=2.5)
    auto_scale_values = [True, False]
...
def main():
    ...
    if args.plot_only:
        ...
        _make_plots(beta_x_values, dtv_results, beta_results, floor, N, h, num_reads, out_dir)
        return
    ...
    _make_plots(args.beta_x_values, dtv_results, beta_results, floor, N, args.h,
                args.num_reads, out_dir)
