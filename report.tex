...
\begin{abstract}
We compare classical samplers, quantum annealers, and physics-inspired hardware within a single rigorous framework for quantum-assisted variational Monte Carlo. A restricted Boltzmann machine represents the wavefunction and is trained with stochastic reconfiguration. We evaluate the following classical and quantum samplers: Metropolis, Gibbs, simulated annealing, an FPGA-based solver, and D-Wave quantum annealers (Pegasus, Zephyr). We report on their behavior across different physical models: the transverse-field Ising model and the (frustrated) Heisenberg chain. We assess sampler performance by two metrics under a matched training protocol: time to a validated energy tolerance, and energy consumed to reach it. FPGA is the fastest and most energy-efficient of the metered classical solvers (we do not measure QPU energy consumption, so no energy comparison is made). On time-to-solution, Pegasus and Zephyr (with an effective-temperature correction applied) both scale nearly flat with $N$ and overtake FPGA by the largest system size tested. However, given the small number of system sizes tested no claims regarding quantum advantage are made. We evaluate a native sparse embedding on the quantum annealer's hardware graph and find that it loses representational capacity. Embedding multiple problem copies in parallel is a promising direction for saving QPU time, though further experiments are needed for a solid claim. The variational ansatz used here is restricted to non-negative amplitudes. We experimentally apply the Marshall sign rule and find that it can extend the variational ansatz' validity to a moderately frustrated Heisenberg chain.
\end{abstract}
...
The internship was conducted at the Institute of Applied and Theoretical Informatics, Polish Academy of Sciences, in Gliwice from March 1st to August 31st, under the supervision of dr hab. Bartłomiej Gardas of the Quantum Systems of Informatics group. The task given was to broaden the work conducted in \cite{Gardas_2018}, to develop a framework to systematically test different samplers used in Boltzmann machines to simulate many-body systems. During the internship, I participated in the KCIK (National Quantum Information Centre) Symposium, which was held in Sopot, Poland, and presented a poster with the title "Quantum-inspired dynamical models on quantum
and classical annealers". All implementations and experiments described in this work were conducted by me, unless otherwise stated.
\section{Overview}
...
where $\boldsymbol{\sigma}_i = (\sigma_i^x, \sigma_i^y, \sigma_i^z)$ are the Pauli spin operators, $J_{ij}$ is the exchange coupling between sites $i$ and $j$, and $\langle i,j \rangle$ denotes nearest-neighbour pairs. Spin operators are represented by Pauli matrices with eigenvalues $\pm1$. In contrast to the \gls{tfim}, the Heisenberg model couples all three spin components.
...
\end{equation}
Gibbs sampling on the D-Wave annealer instead targets the joint distribution $p_{\boldsymbol\theta}(\mathbf{v},\mathbf{h})$ where both $\mathbf{v}$ and $\mathbf{h}$ are encoded as physical qubits (\autoref{fig:rbm-embedding}). The annealer returns samples of the pair $(\mathbf{v},\mathbf{h})$. $\mathbf{h}$ is subsequently marginalized to obtain samples of $\mathbf{v}$ distributed as $\rho(\mathbf{v})$.
...
\item Calculating the \textbf{local energies}: The local energy is given by\cite{Gardas_2018}
\begin{equation}
...
\item Calculating the \textbf{log-derivatives}\cite{Gardas_2018}: Since the ansatz \eqref{eq:ansatz} already has the hidden units traced out, the gradient observable $O_k$ needed for \gls{sr} (\autoref{sec:stochsticreconfig}) can likewise be evaluated from the visible units alone. Writing $\phi_j = b_j + \sum_i W_{ij} v_i$,
\begin{equation}
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption{(a) \gls{rbm} with $12$ visible and $12$ hidden units. (b) Embedding of the corresponding bipartite graph onto the Zephyr hardware topology. Blue nodes are visible units, red nodes are hidden units, orange nodes are auxiliary chain qubits. Each physical node is connected to one auxiliary qubit.}
...
\For{$t = 1, 2, \ldots$}
    \State Sample $\mathbf{v}^{(s)} \sim |\Psi_{\boldsymbol{\theta}}|^2$
           for $s = 1, \ldots, N_s$ \Comment{\gls{mcmc}}
...
\begin{figure}[h]
  ...
  \caption{Spin configurations sampled from a trained \gls{rbm}-VMC model for the one-dimensional \gls{tfim} at three values of the transverse field $h$. Each column shows 2\,000 spin configurations $\boldsymbol{\sigma} \in \{-1,+1\}^N$ sampled from the learned wave function $|\Psi_{\boldsymbol\theta}|^2$, sorted by total magnetisation. The model is a fully-connected \gls{rbm} ($N = N_h = 16$) trained for 300 iterations of \gls{sr} with 500 Metropolis-Hastings samples per step.}
...
\end{equation} Concretely, the \gls{rbm}'s own weights $\boldsymbol\theta=\{\mathbf{a},\mathbf{b},\mathbf{W}\}$ are divided by $\beta_x$ before being embedded on the QPU, i.e.\ $E_\text{in}=E_{\boldsymbol\theta}(\mathbf{v},\mathbf{h})/\beta_x$, so the hardware effectively samples the \gls{rbm}'s joint distribution at $\beta_\text{eff}=\beta_\text{hw}/\beta_x$. Therefore, all problems submitted to the QPU need to be rescaled by a factor $\beta_x$, which we estimate using \gls{cem} (described in the next section).
...
\end{equation}
with $i$ being the sample index. $h_{ij}$ is the observed hidden unit $j$ in sample $i$, and $\Theta_j(\mathbf{v}_i) = b_j + \sum_l v_{i,l}W_{lj}$ with $l$ the visible-unit index. This allows us to perform the \gls{cem} algorithm without additional sampling cost.
...
\begin{figure}[h]
    ...
    \begin{subfigure}{0.48\linewidth}
        ...
        \caption{\gls{cem} vs.\ exact ground truth.}
...
\begin{figure}[h]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \caption{Four independent $K_{8,8}$ \gls{rbm} embeddings ($n_\mathrm{parallel}=4$) placed simultaneously on the Pegasus chip. Colors distinguish copy 1-4; hatched markers denote visible units, solid markers denote hidden units.}
...
It is well known that capturing some quantum dynamics with non-trivial sign structure using a real-valued variational ansatz is not possible (\textit{sign problem})\cite{Troyer_2005}. This does not pose a problem for the \gls{tfim} models, as it is a \textit{stoquastic} model\cite{2011772.2011773}. A stoquastic matrix is defined to have only real and non-positive off-diagonal elements. It can easily be seen that the \gls{tfim} is stoquastic from its definition in  \autoref{eq:tfim}: the $\sigma^z_i\sigma^z_j$ terms only contribute diagonal elements, and the off-diagonal elements from the tranverse field $\sigma^x_i\sigma^x_j$ all contribute the same factor $-h$. Some Heisenberg $J_1$-$J_2$ models do not have this property and the corresponding \gls{rbm} training converges far from the ground state (see \autoref{fig:convergence_median}). 

The reason for this is that the factors  $e^{-\mathbf{a}\cdot\mathbf{v}/2}$ and $\cosh(\cdot)$ of the \gls{rbm} ansatz defined in \autoref{eq:ansatz} are strictly positive for real parameters and therefore the ansatz does not allow for the representation of negative amplitudes\cite{Carleo_2017}.
...
\begin{figure}[htbp]
    ...
    \caption{Energy error per site, $|E_\text{RBM}-E_\text{exact}|/N$, with and without the Marshall sign correction, against $J_2/J_1$ (mean $\pm$ std over seeds). Model trained using Metropolis-Hastings sampling, $N=8$, $200$ \gls{sr} iterations and averaged over 3 seeds, swept over 11 points from $0$ to $1$.}
    \label{fig:marshall-comparison}
\end{figure}

We validate this experimentally in \autoref{fig:marshall-comparison}: applying the Marshall sign correction reduces the ground-state energy error by several orders of magnitude compared to the uncorrected ansatz, confirming that the correction works in practice. Consistent with \cite{Richter_1994}\cite{Voigt_2000}, this improvement persists over a range of frustration $J_2/J_1$ before degrading, verifying that the sign rule can empirically survive frustration beyond its proven regime of $J_2=0$. We did not test a phase-capable ansatz (e.g.\ a complex-valued \gls{rbm}) and leave it to future work.
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{figures/fig_convergence_median.pdf}
    \caption{\gls{rbm} convergence curves ($E/N$ vs.\ \gls{sr} iteration) for
...
\end{equation}
undefined if no such $t^\star$ exists within budget. For a given solver and $N$, let $p$ be the fraction of the $20$ seeds for which $t^\star$ exists, and let
\begin{equation}
\label{eq:tte-Tr}
...
\autoref{fig:tte_self_convergence} (top) plots the median \gls{tte}$_{99}$ (with interquartile range) against system size $N\in\{8,12,16,24,32,64\}$ on three panels: \textbf{(a)} classical \gls{mcmc} samplers (Metropolis, Gibbs); \textbf{(b)} classical, physics-inspired heuristics (\gls{sa} run locally on the GPU described in \autoref{sec:hw_config}, and FPGA); and \textbf{(c)} the Pegasus and Zephyr QPUs with \gls{cem} correction. For budget reasons, we report QPU results at four sizes only: $N\in\{8,16,32,64\}$. 

FPGA is fastest through $N=32$, where every series still validates $20/20$ seeds and \gls{tte}$_{99}$ therefore equals $T_r$ (no retries needed, $p=1$). By $N=64$, however, a handful of seeds no longer validate within budget for several series (FPGA $19/20$, Zephyr(+\gls{cem}) $19/20$, Metropolis $17/20$, Gibbs $16/20$, \gls{sa} $18/20$): FPGA's \gls{tte}$_{99}$ ($0.87\,\mathrm{s}$) is now clearly higher than Pegasus(+\gls{cem})'s. Gibbs stays essentially flat across $N$ (with exponent $\propto N^{+0.65}$). We assume that this is due to a fixed one-time chain initialization (\autoref{tab:classical_sampler_params}), which does not grow with $N$ in this range (\textit{persistent} contrastive divergence).
...
\begin{figure}[h]
    ...
    \caption{\textbf{(top)} Median \gls{tte}$_{99}$ (interquartile range) against system size $N$, under the protocol given in the text. Panels: \textbf{(a)} classical samplers, \textbf{(b)} classical physics-inspired heuristics, \textbf{(c)} quantum annealers (QPU, with \gls{cem} correction). Markers are annotated with the validated-over-total seed count whenever that fraction is below $20/20$. Dotted lines show each series' fitted power law $\propto N^p$, with $p$ given in the legend. \textbf{(bottom)} Median GPU energy$_{99}$ to the same validated-convergence cells, panels (a)--(b) only}
    \label{fig:tte_self_convergence}
\end{figure}

\section{Conclusion}
We assessed classical, quantum-annealing, and physics-inspired samplers within one framework, across the transverse-field Ising and Heisenberg models, using both time-to-solution and energy consumption. FPGA is the fastest and most energy-efficient of the metered classical solvers; QPU energy was not measured, so no classical-versus-quantum energy comparison is claimed. On time-to-solution, under the $99\%$-confidence-corrected \gls{tte}$_{99}$ metric, Pegasus(+\gls{cem}) and Zephyr(+\gls{cem}) both scale nearly flat with $N$ and overtake FPGA by $N=64$, reversing the near-tie a plain-median reading would suggest. This is not established \textit{quantum advantage}, given the small number of system sizes tested and the excluded network/embedding overhead. We also assessed a native sparse embedding onto the annealer's hardware graph, which costs more accuracy than it saves in overhead. We investigated parallel embedding of multiple problem copies on the QPU and find it a promising direction for saving QPU time, though the seed count collected here is still small. We find that the variational ansatz used in this work cannot represent ground states with sign structure and analyze the impact of the Marshall sign rule, which restores accuracy for weakly frustrated Heisenberg chains, but accuracy degrades beyond moderate frustration.

Overall, FPGA remains the strongest sampler across the range of sizes tested here; the QPUs' late, criterion-sensitive overtake at $N=64$ is a lead worth following up.
% -------------------- Appendix -------------------- *
\clearpage
\appendix
...
\label{sec:hw_config}
All classical computation (classical and training loop) run locally via JAX on a single NVIDIA TITAN RTX GPU (24\,GB, driver 560.35.03, CUDA 12.6). The FPGA backend runs on external hardware. Access to the FPGA with a pre-installed sampler was provided by the institute. D-Wave experiments use the \texttt{Advantage\_system6} (Pegasus) and \texttt{Advantage2\_system1} (Zephyr) solvers and were run between 2026-05-11 and 2026-08-10. Forward-anneal runs use a $20\,\mu\mathrm{s}$ anneal time and \texttt{num\_reads} equal to the sample count, with \texttt{auto\_scale} disabled and with D-Wave's default chain strength. Embeddings are cached per \texttt{(n\_visible, solver)} using \texttt{minorminer}'s biclique embedder. FPGA runs use \texttt{Float32} precision, a $120\,\mathrm{MHz}$ core clock, and 1024 parallel replicas. Its reported time is an on-device cycle count divided by this clock frequency. All stochastic components are seeded per run directly and deterministically from the stated per-run integer seed: \texttt{jax.random.PRNGKey(seed)} is split to separately seed model initialization and the sampler before training starts.

\paragraph{Solver internal parameters.}
Each classical sampler in panels (a)-(b) has its own internal mixing/integration parameters, held fixed across every $(N,\text{seed})$ cell (see \autoref{tab:classical_sampler_params}). Metropolis and Simulated Annealing restart their $200$-chain ensemble from a uniform-random configuration at every \gls{sr} iteration. Gibbs instead uses Persistent Contrastive Divergence (PCD), described in \autoref{sec:gibbs}.
\begin{table}[htbp]
\centering
\small
\begin{tabular}{lccl}
\toprule
Solver & Chain & Sweeps/iteration & Other parameters \\
\midrule
Metropolis-Hastings & fresh & $200$ warmup $+$ $1$ & single-spin-flip proposals \\
Gibbs (PCD) & persistent & $10$ (block) & one-time $200$-sweep init \\
Simulated Annealing & fresh & $200$ warmup $+$ $40$ cooling & $T: 5.0\to1.0$ geometric schedule \\
\bottomrule
\end{tabular}
\caption{Internal parameters for the classical samplers of \autoref{fig:tte_self_convergence}. A sweep is $N$ single-spin-flip (Metropolis/\gls{sa}) or $N$-unit block (Gibbs) proposals.}
\label{tab:classical_sampler_params}
\end{table}

\subsection{Convergence trajectories across models, sizes, and solvers}
\autoref{fig:appendix_size_solver_grid} shows the full training trajectories for the summary numbers in \autoref{fig:tte_self_convergence} by plotting the median energy per \gls{sr} iteration. The top block ($N=8,12,16$) has four columns: the \gls{tfim} solved with classical \gls{mcmc} (Metropolis, Gibbs, Simulated Annealing), the \gls{tfim} solved with the physics-inspired and hardware solvers (FPGA, D-Wave Pegasus+\gls{cem}, Zephyr+\gls{cem}), and the frustrated $J_1$-$J_2$ Heisenberg chain at $J_2/J_1=0.3$ and $0.5$, each solved with Gibbs and Simulated Annealing. The bottom block ($N=24,32,64$) only has the two \gls{tfim} columns, since we did not run the Heisenberg model past $N=16$. All \gls{tfim} panels use the same matched hyperparameters as \autoref{fig:tte_self_convergence} ($h=0.5$, learning rate $0.08$, regularization $0.05$, $200$ samples per iteration, $100$ iterations, $20$ seeds; see \autoref{tab:classical_sampler_params} for the solver-specific settings). The Heisenberg panels are not matched in the same way: each $(N,\text{method},J_2)$ cell instead uses whichever learning rate, regularization, and hidden-unit count we found was best using a prior hyperparameter search for that specific cell.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{figures/fig12_appendix_size_solver_grid.pdf}
    \caption{Median per-iteration energy (IQR band) across system size $N$, physical model, and solver, dashed line marks the exact ground-state energy. Top block: $N=8,12,16$, all four columns. Bottom block: $N=24,32,64$, \gls{tfim} columns only (Heisenberg archive stops at $N=16$). \gls{tfim} columns use the \autoref{fig:tte_self_convergence} matched cell; Heisenberg columns use each cell's own per-instance-tuned hyperparameters (see text).}
