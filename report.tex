...
The Ising model consists of discrete spins arranged on a lattice, interacting via nearest-neighbour couplings. In its quantum extension, the \gls{tfim}, a transverse magnetic field is added, introducing quantum fluctuations through non-commuting terms in the Hamiltonian. The Hamiltonian for the one-dimensional \gls{tfim} is given by
\begin{equation}
...
The Heisenberg model describes spins on lattice sites interacting via exchange coupling~\cite{mpg}:
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption{(a) RBM with $24$ visible and $24$ hidden units. (b) Embedding of the corresponding bipartite graph onto the Zephyr hardware topology. Blue nodes are visible units, red nodes are hidden units, orange nodes are auxiliary chain qubits. Each physical node is connected to one auxiliary qubit.}
...
As can be seen from \autoref{fig:dtv_autoscale_dtv_N8}, enabling autoscaling dramatically worsens sampling performance. Therefore, all other D-Wave experiments in this work use \texttt{auto\_scale=False}
...
To disentangle how much of this degradation is a fundamental limit of the sparse ansatz itself versus an artefact of MCMC training, we additionally trained the same four masks ($N=16$, $h=1$, zephyr, $\alpha=1$) via exact enumeration of all $2^{16}$ visible configurations weighted by the exact $|\Psi(v)|^2$ probability, removing all sampling noise and MCMC bias from the optimization. This exact-ansatz floor's mean energy error per spin, $|\varepsilon|/N$ (the quantity plotted in \autoref{fig:sparsity-line}), is $1.05\%$, $1.48\%$, $3.45\%$, and $2.41\%$ across the four masks (sparsity $0.557$ to $0.877$). Rather than staying flat, it roughly triples before falling back, peaking at the third mask. The classical-MCMC curve, by contrast, has a mean per-spin error that rises from $13.4\%$ to $28.5\%$ over the same range, which is $8$ to $17$ times larger than the floor depending on the mask. The spread across seeds also grows sharply with sparsity, while the floor's own seed-to-seed spread stays comparatively small at low sparsity. This indicates that most of the degradation visible in \autoref{fig:sparsity-simple} is driven by optimization/sampling difficulty rather than by the sparse ansatz running out of representational power. This ablation was run at a single system size ($N=16$) and one specific hardware subgraph per sparsity level, so it should be read as an indicator, and not as a general claim about all sparsity regimes.
\begin{figure}[htbp]
    \centering
...
To speed up calculation and save computing time, it is possible to embed multiple smaller, independent problems on the QPU. For example, it is possible to gather 1000 independent samples using only 250 annealing runs, by embedding the \gls{rbm} four times on the QPU. We experimentally validate this approach with 3 seeds per $n_\mathrm{parallel} \in \{1,3,5\}$, shown individually rather than aggregated to a single "best" run per level, as \autoref{fig:parallel-embedding} makes visible that training at this iteration budget is not reliable: 4 of the 9 runs diverge (crossed-off markers), including 2 of 3 seeds at $n_\mathrm{parallel}=5$. Among the runs that do converge, higher $n_\mathrm{parallel}$ still reaches the same energy plateau using markedly less cumulative QPU time (e.g.\ $13.6\,\mathrm{s}$ at $n_\mathrm{parallel}=1$ versus $5.7\,\mathrm{s}$ at $n_\mathrm{parallel}=5$), so the core speedup claim holds. With only 3 seeds per level, however, the divergence rate itself should be read as suggestive, not a precise reliability estimate.
\begin{figure}[h]
    \centering
...
For certain Heisenberg models with bipartite lattice structure, the Marshall sign rule can be used to remove the non-trivial sign structure of the ground state wavefunction by basis transformation. Considering the following unitary transformation on bipartite lattices $A \cup B$, written in the Pauli convention used throughout this work:
\[
...
We next compare solvers by \gls{tte}, the wall-clock time until training reaches a \emph{validated} convergence, defined as an energy error per spin below $\epsilon=0.1$. A practitioner deploying one of these solvers on a new problem has no access to the exact ground-state energy, so a useful stopping rule must be oracle-free: we monitor the coefficient of variation $\mathrm{CV}=\mathrm{std}(E_\mathrm{loc})/|\mathrm{mean}(E_\mathrm{loc})|$ of the local energy and mark a run as self-converged once $\mathrm{CV}$ has stayed below $0.05$ for $10$ consecutive iterations. A run only counts as validated, however, if the energy averaged over that self-detected plateau is, in fact, within $\epsilon$ of the true ground-state energy. Some D-Wave cells at small $N$ self-detect convergence at the wrong plateau; such runs are censored rather than counted, so that \gls{tte} reports time to a genuinely good answer rather than merely time to training appearing to stop.

Every series shares the same training protocol: learning rate $0.08$, regularization $0.05$, $200$ samples per iteration, a $100$-iteration budget, and $20$ seeds per cell. This is a matched-hyperparameter comparison, not a matched-tuning-effort one, and should be read as such. For the D-Wave runs, the reported time is the QPU's own \texttt{qpu\_access\_time} (on-chip programming, anneal, and readout), with network latency and queueing excluded and embedding treated as a one-time cost paid outside the training loop. The comparison should also not be over-read as temperature-comparable: the effective inverse temperature $\beta_\text{eff}$ of the QPU runs is not independently calibrated here (see \autoref{sec:exper:temp} for how $\beta_\text{eff}$ is estimated elsewhere in this work).

\autoref{fig:tte_self_convergence} (top) plots the median \gls{tte} (with interquartile range) against system size $N\in\{8,12,16,24,32,64\}$, split across three panels sharing a $y$-axis so that absolute scale stays directly comparable: \textbf{(a)} classical MCMC samplers with an asymptotic-correctness guarantee (Metropolis, Gibbs); \textbf{(b)} classical, physics-inspired heuristics with no such guarantee (\gls{lsb} with \gls{cem}, \gls{sa} run locally on the GPU described in \autoref{sec:hw_config}, and FPGA); and \textbf{(c)} the Pegasus and Zephyr QPUs, each with and without \gls{cem}. Splitting the nine series this way, rather than plotting them on one shared axis, is what keeps the classical-versus-quantum comparison in panel (c) legible. We omit $N=128$ from the figure: at that size the fixed $100$-iteration training budget does not bring any solver within $\epsilon$ of the exact ground-state energy, which we confirmed directly rather than relying on the self-detector, so every series would censor there for the same training-budget reason rather than any real difference in sampler speed. Plotting $N=128$ would therefore invite a solver-versus-solver reading that the data cannot support. The omission should not be mistaken for a QPU coverage gap: Pegasus and Zephyr are both represented at every other size, $N\in\{8,16,32,64\}$.

Across the plotted range, panel (b)'s FPGA is consistently fastest, followed by \gls{sa}, then panel (c)'s two QPUs, with panel (a)'s Metropolis and Gibbs and panel (b)'s \gls{lsb} the slowest. This ordering is a trend rather than a strict rule: \gls{sa} and FPGA become heavily censored at $N=64$ (only $12/20$ and $6/20$ seeds validate there, respectively), and both QPUs overtake them at that size. Most cells validate on most or all of their $20$ seeds, but two are fully censored, both in panel (c): the Zephyr QPU without \gls{cem} validates $0/20$ seeds at $N=8$, and the Pegasus QPU without \gls{cem} validates $0/20$ seeds at $N=16$. Both are plotted as hollow markers at the run's actual elapsed time at the $100$-iteration budget, not an extrapolated value. The \gls{cem}-corrected Pegasus and Zephyr series validate far more consistently at these same sizes, consistent with \gls{cem} correcting the effective-temperature miscalibration discussed in \autoref{sec:exper:temp}.

Since wall-clock time conflates solver speed with hardware efficiency, \autoref{fig:energy_self_convergence} (bottom) reports the same validated-convergence cells in terms of GPU energy consumed instead, restricted to the classical solvers in panels (a) and (b): the D-Wave QPUs are omitted because no API exposes per-job energy draw, and the only published figure (the whole dilution-refrigerator power draw) is not attributable to a single training run. FPGA energy is an assumed-constant-power estimate ($45\,\mathrm{W}$, matching the FPGA vendor figure used elsewhere in this work) rather than a measured value, and is labelled accordingly in the legend.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{figures/fig10c_tte_vs_n_self_convergence.pdf}
    \par\vspace{1.2em}
    \includegraphics[width=0.75\linewidth]{figures/fig10d_energy_vs_n_self_convergence.pdf}
    \caption{\textbf{(top)} Median time-to-$\epsilon$ (interquartile range) for $\epsilon=0.1$ energy error per spin, against system size $N$, under the shared protocol described in the text (lr$=0.08$, reg$=0.05$, $200$ samples/iteration, $100$-iteration budget, $20$ seeds/cell). Solvers are grouped into three panels on a shared $y$-axis: \textbf{(a)} classical samplers, \textbf{(b)} classical physics-inspired heuristics, \textbf{(c)} quantum annealers (QPU). Filled markers give the median over seeds whose self-detected convergence is validated against the true ground-state energy; a marker is annotated with the validated-over-total seed count whenever that fraction is below $20/20$. Hollow markers mark a cell where no seed validates and are placed at the run's actual elapsed time at the iteration budget rather than an extrapolated value. Pegasus and Zephyr times in panel (c) are D-Wave's \texttt{qpu\_access\_time} only, excluding network/queueing time and the one-time embedding cost. $N=128$ is omitted for every solver because the training budget, not any solver's sampling speed, is the bottleneck at that size. \textbf{(bottom)} Median GPU energy to validated convergence (interquartile range) for the same cells, protocol, and censoring rule, restricted to panels (a)--(b): classical samplers and classical physics-inspired heuristics. Metropolis, Gibbs, and LSB(+CEM) energies come from measured \texttt{gpu\_energy\_wh}, rescaled to the convergence iteration by its share of the run's total sampling time; Simulated Annealing likewise uses measured \texttt{gpu\_energy\_wh}; FPGA uses an assumed constant power draw of $45\,\mathrm{W}$ (labelled ``assumed'' in the legend) times its own measured sampling time, since it was never GPU-metered. D-Wave QPU panels are omitted (see text). Annotations and hollow-marker conventions match the top panel.}
    \label{fig:tte_self_convergence}
    \label{fig:energy_self_convergence}
\end{figure}
...
\label{sec:hw_config}
All non-QPU methods -- RBM training itself and the classical Metropolis, Gibbs, \gls{lsb}, and \gls{sa} samplers -- run locally via JAX on a single NVIDIA TITAN RTX GPU (24\,GB, driver 560.35.03, CUDA 12.6); only the D-Wave Pegasus/Zephyr QPU calls and the separate FPGA/VeloxQ backend run on external hardware. D-Wave experiments use the \texttt{Advantage\_system6} (Pegasus) and \texttt{Advantage2\_system1} (Zephyr) solvers. Forward-anneal runs use a $20\,\mu\mathrm{s}$ anneal time and \texttt{num\_reads} equal to the requested sample count, with \texttt{auto\_scale} disabled (see \autoref{sec:autoscale} for why this matters for $\beta_x$) and D-Wave's default chain strength unless stated otherwise; embeddings are cached per \texttt{(n\_visible, solver)} using \texttt{minorminer}'s biclique embedder. Classical simulated-annealing baselines use \texttt{dwave-neal} with 1000 sweeps and a geometric $\beta$ schedule over $[0.01,10]$. Classical Metropolis-Hastings sampling uses a 200-sweep burn-in followed by 1 sampling sweep per \gls{sr} iteration (a sweep being $N$ single-spin-flip proposals). FPGA/VeloxQ~\cite{veloxq} runs use \texttt{Float32} precision, a 100\,MHz core clock, 1024 parallel replicas, and 1000 update steps per run. All stochastic components (JAX/NumPy) are seeded per run via \texttt{np.random.randint} feeding \texttt{jax.random.PRNGKey}.
...
\section{Conclusion}
We assessed classical, quantum-annealing, and physics-inspired samplers within one framework, across the transverse-field Ising and Heisenberg models, using both time-to-solution and energy consumption. The FPGA-based solver is the fastest and most energy-efficient sampler at every size tested. The quantum annealer's time-to-solution scales more favorably with system size than the FPGA's, so its disadvantage narrows as system size grows, even though it does not close within the range tested. Native sparse embedding onto the annealer's hardware graph costs more accuracy than it saves in overhead; parallel embedding of multiple problem copies is a more effective use of quantum-annealer time. The variational ansatz cannot represent ground states with sign structure; the Marshall sign rule restores good accuracy for weakly frustrated Heisenberg chains, but accuracy degrades beyond moderate frustration. Overall, physics-inspired classical hardware is the strongest sampler at current scale, but the quantum annealer's more favorable scaling suggests it may become competitive at larger system sizes.
