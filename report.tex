...
\]
with only positive amplitudes. This flip is exact for every $J_1$ bond, since bipartiteness guarantees each one connects $A$ to $B$. A $J_2$ bond, however, connects two sites of the \emph{same} sublattice so $U$ leaves its sign unchanged. The correction is therefore exact only for $J_2=0$. It has however been show, that the Marshall correction can survive a certain degree of frustration (that is, $J_1, J_2 \geq 0$)\cite{Richter_1994,Voigt_2000}, which we validate experimentally.
...
We measure the impact of the sign problem on a given probability distribution by the average sign, the standard figure of merit for the severity of a sign problem\cite{Troyer_2005}; there it is defined as a ratio of partition functions for a finite-temperature path-integral ensemble, whereas here the same ratio is applied directly to a $T=0$ ground-state wavefunction, evaluated on the Marshall-rotated exact ground-state wavefunction $\phi = U|\Psi_0\rangle$ obtained by exact diagonalisation:
  ...
  \end{equation}
Evaluated instead on the untransformed $\Psi_0$, this quantity is identically zero for any singlet ground state by SU(2) symmetry ($\sum_{\mathbf{v}}\Psi_0(\mathbf{v})$ vanishes) and is therefore uninformative in that basis; \autoref{fig:marshall-comparison-sign} shows both. We do not evaluate $\langle s\rangle$ on the RBM ansatz $\Psi_\theta$, which is strictly positive by construction and would give $\langle s\rangle \equiv 1$ trivially.

\begin{figure}[htbp]
    \centering
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/marshall_comparison/marshall_comparison_energy.pdf}
        \caption{}
        \label{fig:marshall-comparison-energy}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/marshall_comparison/marshall_comparison_sign.pdf}
        \caption{}
        \label{fig:marshall-comparison-sign}
    \end{subfigure}
    \caption{\textbf{(a)} Energy error per site, $(E_\text{RBM}-E_\text{exact})/N$, with and without the Marshall sign correction, against $J_2/J_1$ (median and IQR over seeds). Without the correction, the ansatz's positive-amplitude restriction causes large, roughly constant error across the whole range; with it, error stays near zero up to the Majumdar-Ghosh point ($J_2/J_1=0.5$) before rising. \textbf{(b)} The average sign $\langle s\rangle$ in the Marshall-rotated basis versus the untransformed basis: the latter is flat at zero everywhere, confirming it carries no information; the former tracks the same breakdown seen in panel (a).}
    \label{fig:marshall-comparison}
\end{figure}

Interestingly, we empirically observe that even for frustrated Heisenberg models with $0\leq\frac{J_2}{J_1}\leq0.5$, the \gls{rbm} with the Marshall sign correction can accurately approach the ground state energy, whereas without the correction the variational energy degrades significantly (\autoref{fig:marshall-comparison-energy}). This is consistent with \cite{Richter_1994}'s square-lattice result and \cite{Voigt_2000}'s direct calculation for the 1D chain, both finding that the sign rule can empirically survive frustration over part of the parameter range, even though the proof no longer strictly applies beyond $J_2=0$. Empirically, we observe that the median relative error over independent seeds rises sharply once $J_2/J_1$ exceeds $0.5$ (\autoref{fig:convergence_median}), in line with expectations that stronger frustration increasingly violates the positive-amplitude assumption underlying the ansatz. We did not test a phase-capable ansatz (e.g.\ a complex-valued \gls{rbm} or a separate amplitude-and-phase network); such a comparison is left as future work.
\begin{figure}[htbp]
    \centering
