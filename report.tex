...
\title{\includegraphics[height=1.8cm]{figures/logo/sorbonne-sciences.png}\hspace{2em}\includegraphics[height=1.8cm]{figures/logo/iitislogo.png}\\[1em]Toward Quantum Competitiveness in Adiabatic Quantum Computing: A Comparative Study of Variational Algorithms}

\begin{document}
\maketitle

\begin{abstract}
We compare classical samplers, quantum annealers, and physics-inspired hardware within a single rigorous framework for quantum-assisted variational Monte Carlo. A restricted Boltzmann machine represents the wavefunction and is trained with stochastic reconfiguration. We evaluate the underlying samplers -- Metropolis, Gibbs, simulated annealing, Langevin Simulated Bifurcation, an FPGA-based solver, and D-Wave quantum annealers (Pegasus, Zephyr) -- on their behavior across different physical models: the transverse-field Ising model and the (frustrated) Heisenberg chain. We assess sampler performance by two metrics under a matched training protocol: time to a validated energy tolerance, and energy consumed to reach it. FPGA is the fastest and most energy-efficient of the metered classical solvers; no QPU energy measurement exists, so no classical-versus-quantum energy comparison is claimed. On time-to-solution, Pegasus(+\gls{cem}) becomes statistically indistinguishable from FPGA by the largest system size tested; Zephyr(+\gls{cem}) alone shows a weak favorable scaling trend, too sensitive to the convergence criterion's parameters to support a general quantum-competitiveness claim. A native sparse embedding onto the quantum annealer's hardware graph loses representational capacity; embedding multiple problem copies in parallel is a promising direction, but with only three seeds per level and a single converging run at the highest parallelism tested, the data collected here cannot support a quantitative speedup claim. The variational ansatz is restricted to non-negative amplitudes; for the frustrated Heisenberg chain, the Marshall sign rule extends its validity up to moderate frustration.
\end{abstract}
...
\label{eq:tts}
\end{equation}
where $P_\text{success}$ is the probability of obtaining the ground state in a single run, and $T_r$ denotes the runtime of a single run. It is important to note that current D-Wave devices exhibit limited connectivity. As a consequence, most problem instances need to be adapted in an additional \textit{embedding} step, increasing the number of physical qubits required. \autoref{fig:tta-native} shows the resulting TTS for the native, embedding-free formulation of one such Hamiltonian; classical solvers dominate across the entire range tested ($N=16$ to $112$), with no crossing point.

A cryptographically motivated instance of the same trend is integer factorisation on the D-Wave Annealer, i.e.\ recovering the two prime factors $p,q$ of a semiprime $N = p \times q$. \cite{willsch2024state} benchmark three \gls{qubo} encodings of this problem -- a direct cost function on the unknown bits (Direct), a multiplication-circuit encoding (MC), and a custom-embedded controlled-full-adder encoding (CFA) -- against uniformly random guessing. \autoref{fig:tta-factoring} reproduces their result: all three encodings still scale exponentially in the number of unknown bits $l = l_p^* + l_q^*$, and only the custom-embedded CFA encoding, which avoids the costly generic embedding step, asymptotically beats random guessing.
...
\caption{Two independent illustrations of a related trend on current QPU hardware, on problems and metrics distinct from the rest of this work (native \gls{qubo} encodings solved directly on the annealer, evaluated by \gls{tts}, rather than this paper's RBM-VMC training evaluated by \gls{tte}). \textbf{(a)} TTS99 for simulating a native single-qubit rotation, governed by $H_1 = \frac{\pi}{2}\sigma_y$, vs.\ system size $N$ \cite{rrf3-jm5m}: classical solvers dominate throughout, with exponential scaling $\propto \exp(N/\beta)$; missing points indicate zero success probability. \textbf{(b)} Success frequency of factoring a semiprime $N = p \times q$ vs.\ the number of unknown bits $l = l_p^* + l_q^*$, reproduced from \cite{willsch2024state}, for the Direct, MC, and CFA \gls{qubo} encodings and for uniformly random guessing; markers are the mean success frequency over $\sim\!10$ random semiprimes per $l$, bars span the 25th--75th percentile, and lines are fitted exponential scalings $\propto 2^{-bl}$. Most encodings scale no better than random guessing; the custom-embedded CFA encoding is the exception (see text).}
\label{fig:tta_overview}
\end{figure}

As previously shown, results returned by the quantum annealer are inherently noisy. This makes it difficult to sample the \textit{exact} ground state of the submitted Hamiltonian. In this work, we therefore assess the device's ability to act as a Gibbs sampler and sample from the Boltzmann distribution. It has been shown earlier that the D-Wave device can in fact be used as a Gibbs sampler\cite{Mehta_2025,Nelson_2022}. Here, we systematically evaluate the quality and time of these samples on computationally hard problems, which allows us to rigorously compare the quantum annealer to classical physics inspired algorithms.
\section{Methods}
...
The Heisenberg model describes spins on lattice sites interacting via exchange coupling~\cite{Heisenberg_1928}:
...
\end{itemize}
\begin{figure*}[ht]
\subfloat[\label{fig:anneal-schedule}]{\includegraphics[width=0.33\linewidth]{figures/anneal_schedule_plot.pdf}}%
    \subfloat[\label{fig:anneal-pegasus}]{\includegraphics[width=0.33\linewidth]{figures/pegasus.pdf}}%
    \subfloat[\label{fig:anneal-zephyr}]{\includegraphics[width=0.33\linewidth]{figures/zephyr.pdf}}
...
Simulated Bifurcation (SB)~\cite{Goto_2019} is a heuristic algorithm that numerically simulates adiabatic and ergodic (chaotic) evolutions of classical nonlinear Hamiltonian systems to solve combinatorial optimization problems. Inspired by quantum adiabatic optimization using networks of \Gls{kpo}, the algorithm encodes the ground state of an Ising Hamiltonian into the stable states of a network of classical oscillators.
...
\gls{lsb} is a stochastic adaptation of SB designed to function as a Boltzmann sampler for training energy-based models such as \gls{srbm}, which allow connections between visible units.  While SB is a deterministic optimization method, LSB introduces stochasticity to approximate a Boltzmann distribution $B_{\beta_\text{eff}}$ with an unknown effective inverse temperature $\beta_\text{eff}$. To handle the unknown $\beta_\text{eff}$ during learning, LSB is often combined with \gls{cem}. A detailed analysis of different temperature parameters is provided in \autoref{sec:exper:temp}.
\paragraph{Conditional Expectation Matching}
\gls{cem} estimates the sampler's effective inverse temperature $\beta_\text{eff}$ by matching the sampler's observed hidden-unit statistics, at a fixed visible configuration, against their closed-form conditional expectation~\cite{kubo2025unlockingpowerboltzmannmachines}; the full derivation, including the pooled variant used in this work, is given in \autoref{sec:exper:temp}. \gls{cem} can also be used with other temperature-dependent solvers, e.g. the D-Wave Quantum Annealer.
...
\subsubsection{Simulated Annealing and Gibbs sampling}
\gls{sa}~\cite{Kirkpatrick_1983} is the classical analogue and predecessor of quantum annealing. It works by exploring the neighborhood of a given configuration, accepting the proposed solution with a temperature dependent probability, or if the new configuration has a lower energy than the current one.
Gibbs sampling~\cite{Geman_1984} can be considered a special case of the Metropolis-Hastings algorithm. It is used when sampling directly from a joint distribution is difficult, but sampling from the full conditional distributions is tractable. The algorithm constructs a Markov chain by iteratively sampling each variable (or block of variables) from its distribution conditioned on the current values of all other variables. This procedure generates a chain whose stationary distribution is the target joint distribution.
...
In the adiabatic case, the ansatz is defined via an energy function over visible and hidden units,
\begin{equation}
...
\end{equation}
with weights $\boldsymbol\theta=\{\mathbf{a}, \mathbf{b}, \mathbf{W}\}$ respectively applied to the vector of visible units $\mathbf{v}=(v_1, \ldots, v_N)\in\{-1,+1\}^N$ and hidden units $\mathbf{h}=(h_1, \ldots, h_M)\in\{-1,+1\}^M$. Both are Ising-valued rather than the more common $\{0,1\}$ Bernoulli convention; this is what gives \autoref{eq:ansatz}'s $2\cosh(\cdot)$ factor and the conditional expectation $\langle h_j\rangle=\tanh(\beta\Theta_j)$ used in \autoref{sec:exper:temp} their specific form. Throughout, $\mathbf{W}\in\mathbb{R}^{N\times M}$ is indexed visible-by-hidden, $W_{ij}$ with $i$ the visible and $j$ the hidden index, matching its representation in the implementation. The joint quantum state is
\begin{equation}
\label{eq:rawansatz}
...
Tracing out the hidden variables in \autoref{eq:rawansatz}, the ansatz becomes:
\begin{equation}
\label{eq:ansatz}
\Psi(\mathbf{v}) = e^{-\mathbf{a} \cdot \mathbf{v}/2} \prod_{j=1}^{N_h} \left[ 2\cosh\!\left(b_j + \sum_i W_{ij} v_i\right) \right]^{1/2}.
\end{equation}
Two sampling regimes are used in this work. Classical Metropolis-Hastings sampling operates directly on the closed-form marginal $\Psi(\mathbf{v})$ of \autoref{eq:ansatz}: only the visible units are instantiated, and $\mathbf{h}$ never appears as an explicit random variable. Gibbs sampling on the D-Wave annealer instead targets the joint distribution $p_{\boldsymbol\theta}(\mathbf{v},\mathbf{h})$: both $\mathbf{v}$ and $\mathbf{h}$ are encoded as physical qubits (\autoref{fig:rbm-embedding}), the annealer returns samples of the pair $(\mathbf{v},\mathbf{h})$, and $\mathbf{h}$ is subsequently discarded (marginalized) to obtain samples of $\mathbf{v}$ distributed as $\rho(\mathbf{v})$. Since D-Wave qubit biases and coupler strengths are restricted to a fixed hardware range, $\mathbf{a}, \mathbf{b}, \mathbf{W}$ are rescaled by a common factor $\beta_x$ before being programmed onto the device. Under \texttt{auto\_scale=False}, this rescaling is the origin of the effective inverse temperature $\beta_\text{eff}$ discussed in \autoref{sec:exper:temp}; \autoref{sec:exper:temp} shows this mechanism no longer holds once \texttt{auto\_scale} is enabled.
\subsection{Boltzmann machine}
A \gls{rbm} is an artificial neural network consisting of visible and hidden units that form a bipartite graph. That is, no intra-layer connections are allowed. During training, the \gls{rbm} learns to approximate a probability distribution.
The main idea is that expectation values with respect to the quantum state can be estimated using samples drawn from the probability distribution $|\Psi(\mathbf{v})|^2$. Since the wavefunction ansatz is represented by a Restricted Boltzmann Machine, which corresponds to a classical stochastic Ising model, configurations $\mathbf{v}$ can be sampled approximately using a D-Wave quantum annealer.
...
We use the following workflow to determine $D_\mathrm{TV}$: First, we use a \gls{sa} pretrained \gls{rbm} and compute its exact distribution $|\Psi|^2$ numerically by enumerating all possible spin configurations. We then collect samples from different samplers and compare the resulting empirical histogram to the exact distribution.

The results are presented in \autoref{fig:dtv_classical_N8_M8}. As a sanity check, it can be seen that Metropolis and Gibbs sampling perform theoretically near-optimal, as is expected. We observe that \gls{lsb} significantly underperforms the other solvers for this small-scale example. The performance of both \gls{lsb} and \gls{sa} improves as the transverse field bias $h$ increases. In \autoref{fig:dtv_classical_N8_M8}\textbf{(b)} the exact histogram of configurations $|\Psi(\mathbf{v})|^2$ is plotted. For $h=0.5$ the distribution is peaked around a handful of highly probable sample configurations, whereas the distribution becomes more uniform for stronger values of $h$. This explains the performance increase for \gls{lsb} and \gls{sa} samplers.

The peaked distribution originates from the fact that the spins in the \gls{tfim} description given by \autoref{eq:tfim} are aligned for low $h$ values (\textit{ferromagnetic} phase). For $h\rightarrow\infty$ the transverse field dominates and the ground state becomes a product of single-spin $\sigma_x$ eigenstates. Measured in the z-basis, this leads to an exactly uniform distribution over spin configurations for any $N$. \autoref{fig:tfim_ordering} illustrates this directly: sampled spin configurations and correlation functions at three values of $h$, spanning the ferromagnetic and paramagnetic regimes.
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{figures/spin_ordering_across_qpt}
  \caption{Spin configurations sampled from a trained RBM-VMC model for the one-dimensional Transverse-Field Ising Model (TFIM) at three values of the transverse field $h$. \textbf{Top row:} each column shows 2\,000 spin configurations $\boldsymbol{\sigma} \in \{-1,+1\}^N$ sampled from the learned wave function $|\Psi_{\boldsymbol\theta}|^2$, sorted by total magnetisation. \textbf{Bottom row:} spin-spin correlation $C(r) = \langle \sigma^z_0 \sigma^z_r \rangle$ as a function of distance $r$, computed from VMC samples (solid line) and exact diagonalisation (dashed line). The relative energy error $\varepsilon = |E_\mathrm{VMC} - E_\mathrm{exact}|/|E_\mathrm{exact}|$ is shown in each panel. The model is a fully-connected RBM ($N = N_h = 16$) trained for 300 iterations of Stochastic Reconfiguration with 500 Metropolis-Hastings samples per step.}
...
In this section, we evaluate the influence of temperature on different models and solvers. We focus on two quantities: the rescaling factor $\beta_x$ and the effective temperature $\beta_\text{eff}$~\cite{Benedetti_2016}. Sampling from an ansatz, all weights in the \gls{rbm} are rescaled by $\beta_x$, that is, the sampler is programmed with $E_\text{in}=\alpha E_{\boldsymbol\theta}$, $\alpha=1/\beta_x$. If the device samples at hardware inverse temperature $\beta_\text{hw}$, it draws from $p(\mathbf{v},\mathbf{h})\propto e^{-\beta_\text{hw}E_\text{in}(\mathbf{v},\mathbf{h})}=e^{-(\alpha\beta_\text{hw})E_{\boldsymbol\theta}(\mathbf{v},\mathbf{h})}$, i.e.\ relative to the unscaled model $\beta_\text{eff}=\alpha\beta_\text{hw}$. For an ideal Gibbs sampler held at $\beta_\text{hw}=1$, the following equation holds:
\begin{equation}
\beta_\text{eff}=\frac{1}{\beta_x}.
\end{equation}
\subsubsection{Langevin Simulated Bifurcation}
As \gls{lsb} is a temperature dependent solver and not guaranteed to sample from the Boltzmann distribution, it can be used to illustrate the influence of the input temperature $\beta_x$ on the solution quality. Contrary to the quantum annealer, sampling using \gls{lsb} does not involve additional costs. To evaluate the impact of the scaling parameter $\beta_x$, we calculate the total variation distance $D_\mathrm{TV}$ between the samples and the precise Boltzmann distribution by numerical enumeration. \autoref{fig:dtv_beta_scale_N8} shows that the value of $\beta_x$ minimizing $D_\mathrm{TV}$ shifts modestly with the transverse field $h$: the optimum sits at $\beta_x\approx0.580$, $0.560$, $0.480$ for $h=0.5,1.0,2.0$ respectively, a spread of about $20\%$ over a $4\times$ change in $h$. $D_\mathrm{TV}$ rises clearly on both sides of each optimum. $D_\mathrm{TV}$ is minimal near the point where the effective temperature $\beta_\text{eff}$ is 1.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}{\textwidth}
        ...
        \subcaption{Effective inverse temperature $\beta_\text{eff}$ of the LSB samples,
        estimated via KL-argmin (\(\beta_\text{eff}=\arg\min_\beta D_\mathrm{KL}(p_S \parallel p_\beta)\), \autoref{sec:autoscale}). The dashed curve shows the ideal $1/\beta_x$ prediction for a perfect Gibbs sampler, the horizontal dotted line marks the VMC target $\beta_\text{eff}=1$.}
...
\end{figure}
\subsubsection{Auto-Scaling on the D-Wave QPU}
\label{sec:autoscale}
Submitting a problem to a D-Wave QPU adds a rescaling step, \texttt{auto\_scale}, enabled by default (\autoref{sec:hw_config}): the \gls{sapi} renormalizes the submitted $h$ and $J$ to make full use of the solver's allowed coefficient range, based only on their \emph{relative} magnitudes. Since $\beta_x$ rescales $h$ and $J$ \emph{uniformly}, \texttt{auto\_scale} absorbs that rescaling before the anneal even runs. So $\beta_\text{eff}$ should be essentially independent of $\beta_x$ whenever \texttt{auto\_scale=True}, unlike the $\beta_\text{eff}=1/\beta_x$ behaviour observed for \gls{lsb} above.

We verify this on the \texttt{Advantage\_system6} (Pegasus) chip, using a chain-free \gls{rbm} embedded directly onto a native Pegasus biclique so chain breakage cannot confound the result. \autoref{fig:dtv_autoscale_N8} sweeps $\beta_x$ under both settings: with \texttt{auto\_scale=True}, $\beta_\text{eff}$ stays flat at $\approx 2.7$--$3.0$ across a $5\times$ range of $\beta_x$; with \texttt{auto\_scale=False} it tracks $\approx 4.1/\beta_x$, recovering the ideal $1/\beta_x$ line up to a constant prefactor ($\approx 4.1$ in this configuration). 
As can be seen from \autoref{fig:dtv_autoscale_dtv_N8}, the two curves are close at the smallest $\beta_x$ tested (matching their near-identical $\beta_\text{eff}$ there) and diverge as $\beta_x$ increases: \texttt{auto\_scale=False} improves sharply as $\beta_x$ is tuned toward the point where $\beta_\text{eff}$ crosses the VMC target of $1$ (minimum $D_\mathrm{TV}\approx29.5\%$ near $\beta_x\approx5.5$), while \texttt{auto\_scale=True} cannot benefit from this tuning at all, since $\beta_\text{eff}$ stays pinned regardless of $\beta_x$ -- it is this loss of tunability, not the switch itself, that costs sampling quality. Therefore, all other D-Wave experiments in this work use \texttt{auto\_scale=False}
...
\begin{figure}[htbp]
    ...
    \caption{%
      Impact of \texttt{auto\_scale} on $\beta_x$ for a chain-free Pegasus-embedded \gls{tfim} \gls{rbm} ($N=8$, $h=1$). \texttt{auto\_scale=True} (red) makes $\beta_\text{eff}$ independent of $\beta_x$; \texttt{auto\_scale=False} (blue) restores the intended $1/\beta_x$ control.
    }
...
In this work we use a pooled variant of this matching step that avoids the dedicated conditional-sampling calls the original procedure requires. Rather than fixing $\mathbf{v}$ and drawing samples at that one condition, we reuse the ordinary joint samples $(\mathbf{v}_i,\mathbf{h}_i)_{i=1}^{N_s}$ the sampler already produces at each training iteration. Each sample has its own, generally distinct, visible configuration $\mathbf{v}_i$; we fit a single $\beta_\text{eff}$ across the whole batch,
\begin{equation}
...
The matching step is visualized in \autoref{fig:cem_matching}, where the left subfigure shows multiple candidate functions $\tanh(\beta\Theta)$ dependent on $\beta$ that can be used to match the returned samples. The right subfigure describes the corresponding minimization, in which the squared loss function $F(\beta)$ is minimized over $\beta$ (in practice using scipy's \texttt{minimize\_scalar} function).
We validate this matching procedure against an independent, exact ground-truth estimate. That estimate is obtained by minimising the KL divergence between the empirical visible-marginal distribution and the true visible marginal of the $\beta$-rescaled joint distribution, $p_\beta(\mathbf v)\propto e^{-\beta\,\mathbf a\cdot\mathbf v}\prod_j 2\cosh(\beta\Theta_j)$, via exact enumeration. We sweep $N\in\{8,12\}$, $h\in\{0.5,1.0,1.5,2.0\}$, and $\beta_x\in\{0.5,1.0,1.5,2.0\}$, with 5 seeds per condition and three checkpoints during training (see \autoref{fig:cem_validation}). \gls{cem} shows a small, consistently negative bias at every checkpoint, of order $-0.07$ to $-0.08$ when averaged over $\beta_x$ and $h$; this bias stays roughly flat across training rather than shrinking. One draw saturated the fit's optimizer search bound (marked $\times$ in \autoref{fig:cem_validation}(a)); we exclude it from both the \gls{rmse} below and panel (b)'s bias averages. The resulting \gls{rmse} against the exact estimate is $0.107$ ($N=8$) and $0.112$ ($N=12$). Conditioned on $\beta_x$ instead, the bias is largest at low $\beta_x$ ($\beta_x=0.5$, mean bias $\approx-0.19$) and shrinks as $\beta_x$ grows ($\approx-0.02$ at $\beta_x=2.0$).
\begin{figure}[htbp]
    \centering
    \begin{subfigure}{0.48\linewidth}
    ...
    \end{subfigure}
    \caption{%
        Validation of the \gls{cem} $\beta_\text{eff}$ estimator against an independent exact estimate (KL-argmin against the true visible marginal of the $\beta$-rescaled joint Boltzmann distribution), pooled over $N\in\{8,12\}$, $h\in\{0.5,1.0,1.5,2.0\}$, $\beta_x\in\{0.5,1.0,1.5,2.0\}$, and 5 seeds per condition (480 draws total). (a) CEM's estimate vs.\ the exact ground truth; the dashed line marks perfect agreement. One draw (marked $\times$) saturated CEM's optimizer search bound and is excluded from the \gls{rmse} and from panel (b). (b) Bias of the CEM estimate relative to ground truth, averaged over $h$ and $\beta_x$, at three checkpoints during training ($10\%$/$50\%$/$100\%$ of iterations). Error bars show $95\%$ confidence intervals over seeds.
    }
...
\end{figure}
We also looked directly at the effect of sparsity on its own, keeping the number of hidden units equal to the number of visible units and only changing how many of the real couplers are removed from the connectivity mask. When removing couplers, we always removed one from the hidden unit with the highest remaining degree, so that no visible or hidden unit was ever left with zero connections. \autoref{fig:sparsity-simple} shows the resulting energy error as a function of sparsity, for three classical samplers (Metropolis-Hastings, simulated annealing, and persistent-chain Gibbs). In all three cases, error increases sharply as sparsity increases.

To disentangle how much of this degradation is a fundamental limit of the sparse ansatz itself versus an artefact of MCMC training, we additionally trained the same four masks ($N=16$, $h=1$, zephyr, $\alpha=1$) via exact enumeration of all $2^{16}$ visible configurations weighted by the exact $|\Psi(v)|^2$ probability, removing all sampling noise and MCMC bias from the optimization. This exact-ansatz floor's median energy error per spin, $|\varepsilon|/N$ (the quantity plotted in \autoref{fig:sparsity-line}; median rather than mean, since a single outlier seed at the third mask otherwise dominates a 5-seed average), is $0.99\%$, $1.51\%$, $2.16\%$, and $2.44\%$ across the four masks (sparsity $0.557$ to $0.877$): a steady $\sim\!2.5\times$ rise, peaking at the highest-sparsity mask. Metropolis, \gls{sa}, and persistent Gibbs each rise from $\sim\!10\%$ to $\sim\!27$--$29\%$ over the same range, $9.7$--$17.7\times$ the floor depending on solver and mask (no single solver is representative of all three). The spread across seeds also grows sharply with sparsity, while the floor's own seed-to-seed spread stays comparatively small at low sparsity. This indicates that most of the degradation visible in \autoref{fig:sparsity-simple} is driven by optimization/sampling difficulty rather than by the sparse ansatz running out of representational power. This ablation was run at a single system size ($N=16$) and one specific hardware subgraph per sparsity level, so it should be read as an indicator, and not as a general claim about all sparsity regimes.
\begin{figure}[htbp]
    \centering
    ...
    \end{subfigure}
    \caption{Error as a function of sparsity, with the number of hidden units kept equal to the number of visible units in both plots. \textbf{(a)} Relative error $|\varepsilon|/|E_\mathrm{exact}|$ for different values of $h$, trained classically via Metropolis sampling. \textbf{(b)} Energy error per spin, $|\varepsilon|/N$, at $h=1$ across three classical samplers (Metropolis, simulated annealing, and persistent Gibbs), all using identical hyperparameters (300 SR iterations, learning rate $0.05$), against the exact-ansatz floor, which uses noise-free gradients for up to 3000 iterations to escape optimization plateaus. The dotted line marks the exact-ansatz floor at this size. The vertical dashed grey line marks the native hardware floor: the sparsity of the unpruned chain-free embedding for this system size, i.e.\ the fraction of visible-hidden pairs with no real coupler between them before any edges are deliberately removed. All tested sparsity levels lie to its right, since pruning can only remove couplers, never add ones the chip does not have.}
    \label{fig:sparsity-simple}
\end{figure}
We conclude that a native Pegasus/Zephyr embedding is not suitable for efficient \gls{rbm} sampling, for the following reasons. Even in the optimal case, representability of the sparse ansatz is limited: the noise-free, exact-enumeration floor rises about $2.5\times$ as native couplers are pruned away ($0.99\%$ to $2.44\%$ median per-spin error), showing that sparsification costs real representational capacity independent of how the model is trained. In actual training, effects such as finite-sample estimator noise and MCMC/annealing mixing behavior further degrade performance substantially. Across three independent classical samplers (Metropolis-Hastings, simulated annealing, and persistent-chain Gibbs), trained under identical hyperparameters and iteration budgets, the per-spin error at every tested sparsity level sits $9.7$--$17.7\times$ above this floor, with no sampler avoiding the gap. Due to budget constraints, we tested the sparse models only on classical hardware.
\subsubsection{Parallel Embeddings}
To speed up calculation and save computing time, it is possible to embed multiple smaller, independent problems on the QPU~\cite{Pelofske_2022}. For example, it is possible to gather 1000 independent samples using only 250 annealing runs, by embedding the \gls{rbm} four times on the QPU. We experimentally validate this approach with 3 seeds per $n_\mathrm{parallel} \in \{1,3,5\}$, shown individually rather than aggregated to a single "best" run per level, as \autoref{fig:parallel-embedding} makes visible that training at this iteration budget is not reliable: 4 of the 9 runs diverge (crossed-off markers), including 2 of 3 seeds at $n_\mathrm{parallel}=5$. The $13.6\,\mathrm{s}$ and $5.7\,\mathrm{s}$ visible in \autoref{fig:parallel-embedding-qpu-time} are each run's total recorded QPU time over its full $150$-iteration budget, not the time at which it actually reached the energy plateau; applying this work's oracle-based convergence criterion (\autoref{sec:exper:tte}) to the individual trajectories instead gives plateau times of $0.54\,\mathrm{s}$ and $5.17\,\mathrm{s}$ for the two converging $n_\mathrm{parallel}=1$ seeds, versus $0.23\,\mathrm{s}$ for the single converging $n_\mathrm{parallel}=5$ seed. The two $n_\mathrm{parallel}=1$ seeds alone disagree by roughly $10\times$, so with only one converging seed at $n_\mathrm{parallel}=5$ to compare against, this data does not support a reliable speedup estimate; a substantially larger seed count would be needed before drawing a quantitative conclusion. With only 3 seeds per level, the divergence rate itself should likewise be read as suggestive, not a precise reliability estimate.
\begin{figure}[h]
    \centering
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \caption{Energy vs.\ cumulative QPU access time for $n_\mathrm{parallel} \in \{1,3,5\}$, all 3 seeds shown individually. $\times$ markers indicate a run diverged before reaching the plateau; among the seeds that converge, higher $n_\mathrm{parallel}$ reaches the plateau using less real QPU time.}
...
For certain Heisenberg models with bipartite lattice structure, the Marshall sign rule~\cite{Marshall_1955} can be used to remove the non-trivial sign structure of the ground state wavefunction by basis transformation. Considering the following unitary transformation on bipartite lattices $A \cup B$, written in the Pauli convention used throughout this work:
\[
...
\]
with only positive amplitudes. This transformation is exact only for the unfrustrated case $J_2=0$, where the lattice is bipartite; for $J_2>0$ the next-nearest-neighbour bonds connect sites within the same sublattice, breaking bipartiteness, so applying the same sign pattern is a heuristic gauge choice rather than a proven sign-removal\cite{Richter_1994,Voigt_2000}. We measure the impact of the sign problem on a given probability distribution by the average sign, the standard figure of merit for the severity of a sign problem\cite{Troyer_2005}; there it is defined as a ratio of partition functions for a finite-temperature path-integral ensemble, whereas here the same ratio is applied directly to a $T=0$ ground-state wavefunction, evaluated on the Marshall-rotated exact ground-state wavefunction $\phi = U|\Psi_0\rangle$ obtained by exact diagonalisation:
  \begin{equation}
      \langle s \rangle = \frac{\sum_{\mathbf{v}} \phi(\mathbf{v})}{\sum_{\mathbf{v}} |\phi(\mathbf{v})|}
  \end{equation}
Evaluated instead on the untransformed $\Psi_0$, this quantity is identically zero for any singlet ground state by SU(2) symmetry ($\sum_{\mathbf{v}}\Psi_0(\mathbf{v})$ vanishes) and is therefore uninformative in that basis; \autoref{fig:marshall-comparison} (bottom) shows both. We do not evaluate $\langle s\rangle$ on the RBM ansatz $\Psi_\theta$, which is strictly positive by construction and would give $\langle s\rangle \equiv 1$ trivially.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\linewidth]{figures/marshall_comparison.pdf}
    \caption{\textbf{(top)} Energy error per site, $(E_\text{RBM}-E_\text{exact})/N$, with and without the Marshall sign correction, against $J_2/J_1$ (median and IQR over seeds). Without the correction, the ansatz's positive-amplitude restriction causes large, roughly constant error across the whole range; with it, error stays near zero up to the Majumdar-Ghosh point ($J_2/J_1=0.5$) before rising. \textbf{(bottom)} The average sign $\langle s\rangle$ in the Marshall-rotated basis versus the untransformed basis: the latter is flat at zero everywhere, confirming it carries no information; the former tracks the same breakdown seen in the top panel.}
    \label{fig:marshall-comparison}
\end{figure}

Interestingly, we empirically observe that even for frustrated Heisenberg models with $0\leq\frac{J_2}{J_1}\leq0.5$, the \gls{rbm} with the Marshall sign correction can accurately approach the ground state energy, whereas without the correction the variational energy degrades significantly (\autoref{fig:marshall-comparison}, top). This is consistent with \cite{Richter_1994}'s square-lattice result and \cite{Voigt_2000}'s direct calculation for the 1D chain, both finding that the sign rule can empirically survive frustration over part of the parameter range, even though the proof no longer strictly applies beyond $J_2=0$. Empirically, we observe that the median relative error over independent seeds rises sharply once $J_2/J_1$ exceeds $0.5$ (\autoref{fig:convergence_median}), in line with expectations that stronger frustration increasingly violates the positive-amplitude assumption underlying the ansatz. We did not test a phase-capable ansatz (e.g.\ a complex-valued \gls{rbm} or a separate amplitude-and-phase network); such a comparison is left as future work.
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{figures/fig_convergence_median.pdf}
    \caption{RBM convergence curves ($E/N$ vs.\ SR iteration) for
    $N \in \{8,12\}$ and $J_2/J_1 \in \{0.3,0.5,0.7,0.9\}$ on the frustrated
    $J_1$--$J_2$ Heisenberg chain, using Gibbs sampling. The solid line is
    the median over $n$ independent evaluation seeds (annotated per panel);
    the shaded band is the interquartile range. For each panel, the
    hyperparameter configuration was selected beforehand on a disjoint
    tuning set (Optuna trials, tuning seeds excluded from evaluation);
    $\varepsilon_\mathrm{med}$ denotes the median relative error of the
    tail-averaged energy. Where $n$ is reported as $a/b$, $b$ seeds were
    attempted and $b-a$ runs diverged (non-finite energy) under the selected
    configuration and are retained as failures rather than silently dropped;
    divergence is frequent in the strongly frustrated regime
    ($N=8$, $J_2/J_1 \geq 0.7$). The red dashed line marks the exact
    ground-state energy $E_\mathrm{exact}/N$. The median relative error
    rises sharply beyond $J_2/J_1 = 0.5$.}
...
We next compare solvers by \gls{tte}: the sampler time (the sampling call itself, excluding \gls{sr}/\gls{cg}/gradient work, which is identical across solvers) until the rolling-window mean energy error per spin, $|\overline{E}-E_\mathrm{exact}|/N$ averaged over the trailing $10$ iterations, first drops below $\epsilon=0.1$ (a $9.4\%$ relative energy error at this $h$) and stays below it for $10$ consecutive iterations. \gls{tte} is reported as the first iteration of that sustained window. Requiring $10$ consecutive passes, rather than a single crossing, discards runs that briefly and spuriously dip below $\epsilon$ before moving back away from it, a pattern observed in some D-Wave trajectories. Runs that do not satisfy this criterion within the $100$-iteration budget are censored rather than counted. Pegasus and Zephyr are reported here with \gls{cem} correction only; for these series, the reported time also includes \gls{cem}'s $\beta_\text{eff}$ estimation step, timed on the host identically to every other \gls{cem} series, so it is added on top of the QPU's own device-reported \texttt{qpu\_access\_time} rather than being the same kind of measurement.

Every series shares the same training protocol: $N$ hidden units (matching the visible count), learning rate $0.08$, regularization $0.05$, $200$ samples per iteration, a $100$-iteration budget, and $20$ seeds per cell. This is a matched-hyperparameter comparison, not a matched-tuning-effort one, and should be read as such. Concretely, what is timed differs by solver: for the classical samplers, it is a host-side timer placed around the sampler call itself in the training loop, with a separate one-time call executed before training starts solely to trigger JAX's \gls{jit} compilation, so that cost is excluded from every reported iteration. For the D-Wave runs, the reported time is the QPU's own \texttt{qpu\_access\_time} (on-chip programming, anneal, and readout) as returned by D-Wave's SAPI, with network latency and queueing excluded and embedding treated as a one-time cost paid outside the training loop. The comparison should also not be over-read as temperature-comparable: the effective inverse temperature $\beta_\text{eff}$ of the QPU runs is not independently calibrated here (see \autoref{sec:exper:temp} for how $\beta_\text{eff}$ is estimated elsewhere in this work).

\autoref{fig:tte_self_convergence} (top) plots the median \gls{tte} (with interquartile range) against system size $N\in\{8,12,16,24,32,64\}$, split across three panels sharing a $y$-axis so that absolute scale stays directly comparable: \textbf{(a)} classical MCMC samplers with an asymptotic-correctness guarantee (Metropolis, Gibbs); \textbf{(b)} classical, physics-inspired heuristics with no such guarantee (\gls{lsb} with \gls{cem}, \gls{sa} run locally on the GPU described in \autoref{sec:hw_config}, and FPGA); and \textbf{(c)} the Pegasus and Zephyr QPUs with \gls{cem}. Splitting the seven series this way, rather than plotting them on one shared axis, is what keeps the classical-versus-quantum comparison in panel (c) legible. We omit $N=128$ from the figure: at that size the fixed $100$-iteration training budget does not bring any solver within $\epsilon$ of the exact ground-state energy, so every series would censor there for the same training-budget reason rather than any real difference in sampler speed. Plotting $N=128$ would therefore invite a solver-versus-solver reading that the data cannot support. For budget reasons, we report QPU results at four sizes only: $N\in\{8,16,32,64\}$.

FPGA is fastest through $N=32$; by $N=64$ its median ($0.56\,\mathrm{s}$) and Pegasus(+CEM)'s ($0.47\,\mathrm{s}$) are within each other's interquartile range, so the two are not distinguishable as faster/slower at that size. Gibbs stays essentially flat across $N$ (its lowest fitted exponent, $\propto N^{+0.22}$) since its dominant cost is a fixed one-time $200$-sweep persistent-chain initialization (\autoref{tab:classical_sampler_params}), which does not grow with $N$ in this range; Metropolis, \gls{sa}, and \gls{lsb}(+\gls{cem}) have no such fixed cost and grow steeply instead. Validated fractions stay at or above $16/20$ everywhere in this figure; no cell is fully censored.

Since sampler time conflates solver speed with hardware efficiency, \autoref{fig:tte_self_convergence} (bottom) reports the same validated-convergence cells in terms of GPU energy consumed instead, restricted to the classical solvers in panels (a) and (b): the D-Wave QPUs are omitted because no API exposes per-job energy draw, and the only published figure (the whole dilution-refrigerator power draw) is not attributable to a single training run. FPGA energy is an assumed-constant-power estimate ($45\,\mathrm{W}$, matching the FPGA vendor figure used elsewhere in this work) rather than a measured value, and is labelled accordingly in the legend.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{figures/fig10c_tte_vs_n_validated_convergence_eps0.1.pdf}
    \par\vspace{1.2em}
    \includegraphics[width=0.75\linewidth]{figures/fig10d_energy_vs_n_validated_convergence_eps0.1.pdf}
    \caption{\textbf{(top)} Median \gls{tte} (interquartile range) against system size $N$, under the protocol given in the text. Panels: \textbf{(a)} classical samplers, \textbf{(b)} classical physics-inspired heuristics, \textbf{(c)} quantum annealers (QPU, with \gls{cem}). Markers are annotated with the validated-over-total seed count whenever that fraction is below $20/20$. Dotted lines show each series' fitted power law $\propto N^p$, with $p$ given in the legend. Pegasus/Zephyr times are D-Wave's \texttt{qpu\_access\_time} only, excluding queueing and the one-time embedding cost. \textbf{(bottom)} Median GPU energy to the same validated-convergence cells, panels (a)--(b) only; FPGA energy is an assumed-constant-power estimate and D-Wave QPU energy is omitted (see text).}
...
\subsubsection{\texorpdfstring{\gls{tfim}}{TFIM} ground state solving in \texorpdfstring{$O(N)$}{O(N)}}
The 1D \gls{tfim} defined in \autoref{eq:tfim} can be solved exactly in $O(N)$ time, for $J>0$, $h\geq0$, and periodic boundary conditions on the spin chain. This is the regime used throughout this work. Dziarmaga~\cite{Dziarmaga_2005} solves the identical Hamiltonian by mapping it to a system of free fermions via the Jordan--Wigner transformation, followed by a Fourier and Bogoliubov transformation. The periodic boundary conditions split the fermionic Hilbert space into two parity sectors, each restricted to its own set of allowed momenta; for $J>0$, $h\geq0$ the ground state always sits in the antiperiodic sector. Directly applying the quasiparticle dispersion of Eq.~(16) and the ground-state (vacuum) energy of Eq.~(18) in \cite{Dziarmaga_2005} gives the closed-form ground-state energy, with $g=h/J$ (see also \cite{He_2017} for the parity-sector momenta). We denote the dispersion $\omega(k)$ here rather than $\epsilon(k)$, to avoid colliding with the \gls{tte} target $\epsilon$ used elsewhere in this work:
\begin{equation}
...
The variational principle bounds the exact expectation value $\langle H\rangle_\Psi = \langle\Psi_{\boldsymbol\theta}|H|\Psi_{\boldsymbol\theta}\rangle / \langle\Psi_{\boldsymbol\theta}|\Psi_{\boldsymbol\theta}\rangle$ for a fixed parameter vector $\boldsymbol\theta$: it can never fall below the true ground-state energy.
\subsection{JAX}
...
\label{sec:hw_config}
All non-QPU methods -- RBM training itself and the classical Metropolis, Gibbs, \gls{lsb}, and \gls{sa} samplers -- run locally via JAX on a single NVIDIA TITAN RTX GPU (24\,GB, driver 560.35.03, CUDA 12.6); only the D-Wave Pegasus/Zephyr QPU calls and the separate FPGA backend run on external hardware. D-Wave experiments use the \texttt{Advantage\_system6} (Pegasus) and \texttt{Advantage2\_system1} (Zephyr) solvers; no per-run solver calibration version or timestamp is recorded in the archived result files, so exact calibration cannot be pinned, but the underlying data was collected between 2026-05-11 and 2026-08-10 (per commit history), a window over which D-Wave periodically recalibrates a given solver family. Forward-anneal runs use a $20\,\mu\mathrm{s}$ anneal time and \texttt{num\_reads} equal to the requested sample count, with \texttt{auto\_scale} disabled (see \autoref{sec:autoscale} for why this matters for $\beta_x$) and D-Wave's default chain strength unless stated otherwise; embeddings are cached per \texttt{(n\_visible, solver)} using \texttt{minorminer}'s biclique embedder. Classical simulated-annealing baselines run on this same JAX/GPU path (internal parameters in \autoref{tab:classical_sampler_params}); a separate \texttt{dwave-neal}-based \gls{sa} implementation also exists in the codebase but is not used for any result in this paper. Classical Metropolis-Hastings sampling uses a 200-sweep burn-in followed by 1 sampling sweep per \gls{sr} iteration (a sweep being $N$ single-spin-flip proposals). FPGA runs use \texttt{Float32} precision, a $120\,\mathrm{MHz}$ core clock, and 1024 parallel replicas; its reported time is an on-device cycle count divided by this clock frequency, computed by the external FPGA firmware driver rather than measured by this codebase. All stochastic components are seeded per run directly and deterministically from the stated per-run integer seed: \texttt{jax.random.PRNGKey(seed)} is split to separately seed model initialization and the sampler before training starts.

\paragraph{Solver internal parameters.} Beyond the shared protocol stated above (\gls{tfim} $h=0.5$, lr$=0.08$, reg$=0.05$, $200$ samples/iteration, $100$ iterations, seeds $0$-$19$), each classical sampler in panels (a)-(b) has its own internal mixing/integration parameters, held fixed across every $(N,\text{seed})$ cell. These are listed in \autoref{tab:classical_sampler_params} so a reviewer can reproduce them exactly. Metropolis and Simulated Annealing restart their $200$-chain ensemble from a uniform-random configuration at every SR iteration; Gibbs instead runs Persistent Contrastive Divergence (PCD), paying its warmup once when the $200$ chains are first created and reusing them, refreshed by $10$ block-Gibbs sweeps, at every subsequent iteration. Every archived result file also carries its own \texttt{sampler\_config} record (the exact values in \autoref{tab:classical_sampler_params}, resolved at run time rather than assumed from a default) and the \texttt{git\_sha} of the code that produced it. This means the table is checkable against the archive itself, rather than taken on faith.
...
LSB & $1000$ Langevin integration steps (fresh run) & $\delta=0.1$, $\gamma=0.1$, $\sigma^{-2}=1.0$ \\
\bottomrule
\end{tabular}
...
\paragraph{Reproducibility check.} We re-ran a sample of the archived cells behind \autoref{fig:tte_self_convergence} under the current codebase ($N\in\{8,16\}$, seeds $0$-$2$, all four classical solvers), matching $(N,\text{seed},\text{method})$ exactly, and compared the full $100$-entry per-iteration energy trajectory against the archived one. Metropolis, Gibbs, and Simulated Annealing reproduced bit-for-bit in every case tested. LSB did not. One seed matched to within floating-point noise ($<3\times10^{-9}$), but the others diverged from the then-archived values by up to $0.65$ in final trained energy per spin -- an order of magnitude larger than the $\epsilon=0.1$ convergence threshold used throughout this section. This happened even though two independent fresh reruns of the same seed under the current code agreed with each other bit-for-bit. The likely cause is LSB's chaotic Langevin dynamics: it amplifies a single-ULP-level floating-point difference, accumulated over its $1000$ integration steps, into an $O(1)$ divergence. The discrete accept/reject samplers have no equivalent amplification mechanism.
...
$\mathbf{S} = \frac{1}{N_s}\bar{\mathbf{O}}^\top\bar{\mathbf{O}} + \lambda I$,
