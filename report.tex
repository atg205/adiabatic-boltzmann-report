...
\subsubsection{Conditional Expectation Matching}
...
The results are presented in \autoref{fig:dtv_classical_N8_M8}. As a sanity check, it can be seen that Metropolis and Gibbs sampling perform theoretically near-optimal, as is expected. The performance of \gls{sa} improves as the transverse field bias $h$ increases. In \autoref{fig:dtv_classical_N8_M8}\textbf{(b)} the exact histogram of configurations $|\Psi(\mathbf{v})|^2$ is plotted. For $h=0.5$ the distribution is peaked around a handful of highly probable sample configurations, whereas the distribution becomes more uniform for stronger values of $h$. This explains the performance increase for the \gls{sa} sampler.
...
Simulated annealing is a Metropolis-Hastings chain held at a fixed final temperature $\beta_x$; given enough sweeps it converges to the Boltzmann distribution at that temperature~\cite{Kirkpatrick_1983,Metropolis_1953}. \autoref{fig:dtv_beta_scale_N8} demonstrates this experimentally, showing $\beta_\text{eff}$ converge toward the ideal $1/\beta_x$ line, and $D_\mathrm{TV}$ toward its sampling floor, as the number of cooling sweeps increases.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}{\textwidth}
        ...
        \subcaption{Total variation distance $D_\mathrm{TV}$ between \gls{sa}'s empirical
    ...
    \begin{subfigure}{\textwidth}
        ...
        \subcaption{Effective inverse temperature $\beta_\text{eff}$ of \gls{sa}'s samples,
    ...
    \caption{%
        Influence of \gls{sa}'s final-temperature parameter $\beta_x$ on sampling quality for a trained \gls{tfim} \gls{rbm} with $N=8$ visible spins, at three cooling-sweep counts.
...
\paragraph{Reproducibility check.} We re-ran a sample of the archived cells behind \autoref{fig:tte_self_convergence} under the current codebase ($N\in\{8,16\}$, seeds $0$-$2$, all three classical solvers), matching $(N,\text{seed},\text{method})$ exactly, and compared the full $100$-entry per-iteration energy trajectory against the archived one. Metropolis, Gibbs, and Simulated Annealing reproduced bit-for-bit in every case tested.
