...
\textbf{TODO --- write this subsection from the notes below.}
\begin{itemize}
    \item \gls{tte}: wall-clock time to \emph{validated} convergence, $\epsilon=0.1$ energy-error-per-spin.
    \item Convergence rule: coefficient of variation $\mathrm{CV}=\mathrm{std}(E_\mathrm{loc})/|\mathrm{mean}(E_\mathrm{loc})|$ below $0.05$ for $10$ consecutive iterations (oracle-free stopping rule) \emph{and} the windowed mean energy must land within $\epsilon$ of the true answer -- self-detection at the wrong plateau is censored, not counted.
    \item Common protocol across all series: lr$=0.08$, reg$=0.05$, $200$ samples/iteration, $100$-iteration budget, $20$ seeds; note this is matched hyperparameters, not matched tuning effort.
    \item QPU time = D-Wave's \texttt{qpu\_access\_time} only (on-chip programming + anneal + readout); no network/queueing; embedding is a one-time cost outside the loop.
    \item Marker convention: filled = median/IQR over validated seeds; hollow = 0 validated seeds, plotted at the run's actual elapsed time at the iteration budget (not extrapolated); every point annotated validated/total.
    \item QPU size coverage: Zephyr and Pegasus both $N\in\{8,16,32,64\}$ -- Zephyr's biclique embedding does not reach $N=128$ on the current chip, and $N=128$ is now excluded for every series (see below), so this is no longer an asymmetry between the two QPU topologies.
    \item Figure is capped at $N=64$: at $N=128$ the fixed $100$-iteration training budget does not reach $\epsilon$ for \emph{any} solver, verified against the oracle exact-energy check independent of the self-detector -- every series censors there for the same training-budget reason, not a sampler-speed reason, so plotting $N=128$ would invite a solver-vs-solver reading the data cannot support. State this explicitly so a reader doesn't mistake the cutoff for a coverage gap.
    \item Pattern up to $N=64$ (all series have a validated fraction, mostly $10$--$20/20$, at every plotted $N$): FPGA fastest, then Simulated Annealing, then Pegasus/Zephyr QPU, then Metropolis/Gibbs/LSB, roughly in that order across the whole range -- no series is fully censored inside the plotted range.
    \item Caveat: $\beta_\mathrm{eff}$ for the QPU runs is not independently calibrated here -- don't call QPU/classical TTE directly "comparable" without that caveat.
    \item Pick one of $\epsilon$/$\varepsilon$ and use it consistently.
    \item Implementation note: the validated-convergence check originally averaged the energy window \emph{before} the self-detected plateau instead of the plateau itself, which made it censor runs that had actually converged correctly (validated fraction was near $0/20$ almost everywhere). Fixed in \texttt{compute\_validated\_convergence\_iter} (paper\_figures.py) -- the numbers/figure above are post-fix.
\end{itemize}

\textbf{TODO --- caption notes (keep bullet lists out of \texttt{\textbackslash caption\{\}} itself, breaks the build; write full caption as prose below the figure instead):}
\begin{itemize}
    \item Median TTE (IQR) vs.\ $N$, $\epsilon=0.1$, all solvers, matched protocol, 20 seeds/cell, $N\in\{8,12,16,24,32,64\}$.
    \item Filled = validated median; every point annotated validated/total; no series is fully censored within this $N$ range (unlike the earlier version of this figure).
    \item Pegasus/Zephyr both include $N=8$ through $N=64$.
    \item State explicitly that $N=128$ is omitted because the training budget, not any sampler, is the bottleneck there (see notes above) -- do not let a reader infer the cutoff is a QPU coverage limitation.
\end{itemize}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\linewidth]{figures/fig10c_tte_vs_n_self_convergence.pdf}
    \caption{\textbf{TODO} -- write full caption from the notes above.}
...
\end{equation}
where $i$ is now repurposed as the sample index (rather than the visible-unit index it denoted above), $h_{ij}$ is the observed hidden unit $j$ in sample $i$, and $\Theta_j(\mathbf{v}_i) = b_j + \sum_l v_{i,l}W_{lj}$ with $l$ the visible-unit index. This adds no marginal sampler cost -- a decisive advantage when sampling is the scarce resource, as on the D-Wave QPU -- while pooling $N_s\times N_h$ residuals per estimate instead of $N_h$ residuals at a single condition.
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption{%
        The CEM matching step for $N=16$ spins with $h=1.0$. (a) Several candidate $\tanh(\beta\Theta)$ curves against the empirically observed hidden-unit samples. The curve at $\beta_\text{eff}=1.94$ hugs the data most closely and represents the best estimate of the sampler's temperature. (b) The squared loss function to be minimized.
    }
